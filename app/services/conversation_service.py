...
class ConversationService:
    ...
    def get_user_conversations(self, current_user: User, skip: int = 0, limit: int = 100):
        return self.repo.get_by_user(current_user.id, skip, limit)

    def get_conversation_details(self, conversation_id: UUID, current_user: User) -> Conversation:
        conv = self.repo.get_by_id(conversation_id)
        if not conv:
            raise HTTPException(status_code=404, detail="Conversation not found")
        if conv.user_id != current_user.id:
            raise HTTPException(status_code=403, detail="Not authorized to see this conversation")
        return conv
